  ...
  describe "#scan" do
    context "when a virus is found" do
      it "raises a PafsCore::VirusFoundError" do
        expect(clamav).to receive(:execute) { [build(:virus_found)] }
        expect(ClamAV::Client).to receive(:new) { clamav }
    ...
    context "when the virus scanner generates an error" do
      it "raises a PafsCore::VirusScannerError" do
        expect(clamav).to receive(:execute) { [build(:virus_error)] }
        expect(ClamAV::Client).to receive(:new) { clamav }
    ...
    context "when no virii are found" do
      it "returns true" do
        expect(clamav).to receive(:execute) { [build(:virus_clear)] }
        expect(ClamAV::Client).to receive(:new) { clamav }
        expect(subject.scan("path/to/a/clean_file")).to be true
      end
    end
  end
end
