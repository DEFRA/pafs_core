...
RSpec.describe PafsCore::FloodProtectionOutcomes2040Step, type: :model do
  before(:each) do
    ...
    @fpo1 = FactoryBot.create(:flood_protection2040_outcomes, financial_year: 2017, project_id: @project.id)
    @fpo2 = FactoryBot.create(:flood_protection2040_outcomes, financial_year: 2020, project_id: @project.id)
    @fpo3 = FactoryBot.create(:flood_protection2040_outcomes, financial_year: 2030, project_id: @project.id)
  ...
  describe "#update" do
    ...
    let(:params) do
      ActionController::Parameters.new(
        { flood_protection_outcomes2040_step:
    ...
    context "when params are invalid" do
      ...
      it "does not save the changes" do
        expect { subject.update(error_params) }.not_to change { subject.flood_protection2040_outcomes.count }
      end
    end

    context "when params are valid" do
      it "saves the changes" do
        expect { subject.update(params) }.to change { subject.flood_protection2040_outcomes.count }.by(1)
